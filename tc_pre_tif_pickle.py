...
def creat_pred(predictions, labels):
    pred_binary = torch.where(predictions == 1, torch.tensor(1).to(predictions.device), torch.tensor(0).to(predictions.device))

    label_binary = torch.where(labels == 1, torch.tensor(1).to(labels.device), torch.tensor(0).to(labels.device))

    # 创建距离为1个像素的缓冲区
    buffer = torch.nn.functional.max_pool2d(label_binary.float(), kernel_size=3, stride=1, padding=1).to(predictions.device)
...
with open(r'E:\LCQ\DATA\ikonos\pickle\20240624_wujiandu_newlinelabel_tif_buffer5m_multctwo_128-128_img_wubeijing_quchukongzhi_wuzengjia_ikonos_1.pickle', 'rb') as f:
    images = pickle.load(f)
with open(r'E:\LCQ\DATA\ikonos\pickle\20240624_wujiandu_newlinelabel_tif_buffer5m_multctwo_128-128_label_wubeijing_quchukongzhi_wuzengjia_ikonos_1.pickle', 'rb') as f:
    labels = pickle.load(f)
