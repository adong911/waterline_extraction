    ...
    def __init__(self, imgs, labels, buffers, train: False, isnorm='normon'):
        ...
        if self.train:
            self.trans = transforms.Compose([transforms.RandomHorizontalFlip(
                ...
                # 随机仿射变化。degrees选择的度数范围。scale:比例因子区间，从范围a<=比例<=b中随机采样比例。
                transforms.RandomAffine(degrees=(-90, 90), scale=(0.5, 0.5))])
...
def calc_coast_cross_entropy_loss(prediction, target, edge, edge_weight=16, pos_weight=torch.Tensor([1]).cuda()):
    ...
    if len(prediction[edge == 1]) == 0:
        ...
        edge_bce = F.binary_cross_entropy_with_logits(
            prediction[edge == 1], target[edge == 1])
...
class CombinedLoss(torch.nn.Module):
    ...
    def forward(self, prediction, target, edge):
        ...
        coast_cross_entropy = self.coast_cross_entropy_loss(
            prediction, target, edge, edge_weight=self.edge_weight)
...
class CombinedLoss2(torch.nn.Module):
    ...
    def forward(self, prediction, target, edge):
        ...
        coast_cross_entropy = calc_coast_cross_entropy_loss(
            prediction, target, edge, edge_weight=self.edge_weight, pos_weight=self.pos_weight)
...
# 断点保存的模型路径
save_path = r'E:\LCQ\waterline2\20240603\result2\model\FPNunet(DiceBREloss)_bitchsize32_Adam(lr0.0001)_CombinedLoss(a0.9_post0.1)_epoch200_modelstate.pth'

writer = SummaryWriter(
    r'E:\LCQ\waterline2\20240603\result2\logs\FPNunet(DiceBREloss0.9)_200')
...
train_img, test_img, train_label, test_label, train_buffer, test_buffer = train_test_split(
    images, labels, buffers, test_size=0.2, random_state=42)
print('ok:allotdata')
# #----------------------------------------------------------------------------------#
# ---------------------------------自定义数据集--------------------------------------#
train_set = MyDataset1(train_img, train_label, train_buffer,
                       train=False, isnorm='normon')  # list
...
test_set = MyDataset1(test_img, test_label, test_buffer,
                      train=False, isnorm='normon')
# valid_loader = DataLoader(test_set, batch_size=32, shuffle=True,drop_last=True)
train_loader = DataLoader(train_set, batch_size=32,
                          shuffle=True, num_workers=0)
valid_loader = DataLoader(test_set, batch_size=32,
...
edge_weight = 2
criterion = CombinedLoss(dice_bce_weight=0.9, coast_cross_entropy_weight=0.1,
                         postweight=pos_weight.cuda(), edge_weight=edge_weight)

# 学习率调整策略
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
    optimizer, int(1e10), eta_min=1e-5, verbose=False)  # 参数MAX_STEP
...
for epoch in range(start_epoch, epochs+1):  # range(1, n+1)循环n次
    ...
    print(msg.format(epoch, train_loss, train_accuracy, train_pre,
          train_rec,  train_f1, train_dice, train_iou, train_miou))
    msg = '[Epoch {}][valLoss {:.4f}][Acc {:.2%}][Pre1 {:.2%}][Rec1 {:.2%}][f11 {:.2%}][dice1 {:.2%}][iou1 {:.2%}][miou {:.2%}]'
    print(msg.format(epoch, eval_loss, eval_accuracy, eval_pre,
          eval_rec,  eval_f1, eval_dice, eval_iou, eval_miou))

    # 打印训练结果和预计结束时间
    print("Epoch {} finished, took {}. 预计结束时间 {}.".format(
        epoch, time_per_epoch, datetime.now() + expected_time))
...
plt.legend(('accuracy', 'precision', 'recall', 'f1', 'dice', 'iou',
           'miou'), prop={'family': 'SimHei'}, loc='upper right')
plt.xlabel('epoch次数')
